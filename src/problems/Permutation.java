...
public class Permutation {
    ...
    private static void perm(StringBuffer st, int index){
        ...
        if(index <= 0)
            ...
            for (int i = currPos+1; i < st.length(); i++) {//start swapping all other chars with current first char
                swap(st,currPos, i);
                perm(st, index-1);
                swap(st,i, currPos);//restore back my string buffer
            }
        }
    }
